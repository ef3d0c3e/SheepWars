...
public class ExplosiveSheep extends FuseSheep {
    ...
    public static @NonNull ItemStack getItem(final @NonNull CPlayer cp) {
        ...
        meta.setDisplayName(ser.serialize(
            Component.text(cp.getLocale().SHEEPS_EXPLOSIVE_NAME).color(TextColor.color(255, 0, 0))
        ));
        meta.setLore(Util.coloredLore("§7", cp.getLocale().SHEEPS_EXPLOSIVE_DESC));
    ...
    public @NonNull Component getName(@NonNull CPlayer cp) {
        return Component.text(cp.getLocale().SHEEPS_EXPLOSIVE_NAME).color(TextColor.color(255, 0, 0));
    }
